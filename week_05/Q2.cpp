...
int check_string(string input_string)
{
    input_string.append("$");

    stack<int> states; // will contain states
    states.push(0);
    stack<char> chr; // will contain characters pushed into the parse stack

    int i=0, j, l=input_string.length();

    int state_i, char_i;
    pair<char, int> action, action2;

    int p_len;

    while(!states.empty() && i<l)
    {
        if(symbol_to_index.find(input_string[i])==symbol_to_index.end())
            return 0;
        state_i=states.top();
        char_i=symbol_to_index[input_string[i]];

        action=parse_table[state_i][char_i];
        ...
        {
            //return 0;
            if(productions[action.second][2]=='#')
                p_len=0;
            else
                p_len=productions[action.second].length()-2;
            for(j=0; j<p_len; j++)
            {
                states.pop();
                chr.pop();
            }
            chr.push(productions[action.second][0]);
            action2=parse_table[states.top()][symbol_to_index[chr.top()]];
            if(action2.first=='I')
            {
                states.push(action2.second);
            }
            else
                return 0;
        }
...
int main()
{
    ...
    while(1)
    {
        cout<<"Enter 0.Exit 1.To check if a string belongs to a grammar : ";
        cin>>ch;
        if(ch==1)
        {
            ...
            if(check_string(input_string))
                cout<<"Yes, "<<input_string<<" belongs to the grammar.\n";
            else
                cout<<"No, "<<input_string<<" doesn't belong to the grammar.\n";
        }
        else
        {
            break;
